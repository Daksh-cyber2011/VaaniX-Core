...
///   1. Onboarding guard — incomplete onboarding → `/onboarding`.
///   2. Auth guard — protected routes require an authenticated session when
///      Supabase is configured. Public routes are always accessible.
///
/// Reactivity: a [GoRouterRefreshNotifier] wired to the auth session stream
/// triggers a re-evaluation of the guards on sign-in / sign-out. The splash
/// screen performs the first routing decision once bootstrap completes.

import 'package:flutter/material.dart';
import 'package:flutter_riverpod/flutter_riverpod.dart';
import 'package:go_router/go_router.dart';

import 'package:vaanix_app/core/config/app_environment.dart';
import 'package:vaanix_app/core/constants/route_names.dart';
import 'package:vaanix_app/core/providers/app_providers.dart';
import 'package:vaanix_app/core/router/go_router_refresh_notifier.dart';
...
final appRouterProvider = Provider<GoRouter>((ref) {
  ...
  // Keep guards in sync with onboarding flag (SharedPreferences is an in-memory
  // cache, so re-reading it on each redirect yields the latest value).
  final storage = ref.watch(localStorageServiceProvider);

  // Read auth synchronously. The refresh notifier guarantees the router is
  // re-triggered whenever this value changes.
  ...
  final router = GoRouter(
    ...
    routes: [
      ...
      // Each branch preserves its own navigation stack & scroll state.
      // ----------------------------------------------------------
  ...
  // Release the refresh notifier + its stream subscription when the provider
  // is torn down.
...
class _AppShell extends StatelessWidget {
  ...
  void _goBranch(int index) {
    navigationShell.goBranch(
      index,
      // Preserve the existing nested-stack state when revisiting a branch.
