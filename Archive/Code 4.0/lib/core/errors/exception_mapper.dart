...
/// [Failure] types.
///
/// Data sources and repositories throw [AppException] subclasses (or platform
/// exceptions like [DioException]); repositories catch them and translate to
/// [Failure] objects via [ExceptionMapper.toFailure]. Domain and presentation
/// layers then only ever see [Failure]s — never raw exceptions.
///
/// This keeps the architectural rule "exceptions never cross the repository
/// boundary" enforceable in one place.
...
abstract final class ExceptionMapper {
  ...
  ///
  /// - [AppException] subtypes map 1:1 to a semantically equivalent failure.
  /// - [DioException] is normalized into network/server/timeout failures.
  /// - Supabase / Auth exceptions (carrying a `message`) become auth/server
  ///   failures.
  /// - Anything else becomes [UnknownFailure].
  static Failure toFailure(Object error) {
    // ----- Already a domain failure — pass through (defensive) ----------
    ...
    // ----- Anything with a runtime type name hinting at auth -----------
  ...
  /// Pulls a human-readable message from a [DioException] response body or
  /// the error itself, never throwing.
